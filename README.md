...
- `inspect.js` — standalone double-click element inspector (not included in the patch; paste manually into `mainView.js` when debugging)
- `package.json` — declares `@electron/asar` dependency

## Element inspector

`inspect.js` is a standalone IIFE that adds a yellow panel to the top-right of the Claude window. Double-click any element on the page to see 10 ancestor levels with tag, computed font-family / font-size / line-height, and class list.

To use it: extract the asar, paste the contents of `inspect.js` before the `//# sourceMappingURL` line in `mainView.js`, repack, and re-sign. Remove it by re-running `patch.sh` on a clean base (reinstall Claude from DMG first).
